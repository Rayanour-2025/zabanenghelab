...
<template>
  ...
  <div class="border border-gray-200 rounded-xl shadow-lg bg-white overflow-hidden max-w-full">
    ...
    <editor-content 
      ...
      class="prose max-w-none p-4 min-h-[300px] **max-h-[500px] overflow-y-auto custom-scrollbar** focus:outline-none transition-shadow" 
...
.custom-scrollbar { /* نام کلاس را از custom-offcanvas3 به custom-scrollbar تغییر دادیم یا آن را اضافه کردیم */

  overflow-y: auto;

  /* max-height: calc(100vh - 60px); */ /* این خط را می توان حذف کرد چون max-height را در template با Tailwind اعمال کردیم */

  padding-top: 30px;  /* اگر نمی خواهید پدینگ اضافی داشته باشد، این خطوط را حذف کنید */
...
/* ... استایل custom-offcanvas3 اصلی (در صورتی که هنوز به آن نیاز دارید) ... */
