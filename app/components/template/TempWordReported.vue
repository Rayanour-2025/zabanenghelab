<template>
    <div dir="rtl" class="bg-[#F5F6F4] w-full md:p-8 p-4  rounded-[55px]">
        ...
        <div class="md:mt-3 mt-2 text-xs md:text-base">لغت نامه: <span class="text-[#7FB77E]">{{ reportData.word.dictionary.name }}</span></div>
        <div class="md:mt-5 mt-2 text-xs md:text-base">
            <h3>توضیحات:</h3>
        ...
        <div class="md:mt-5 mt-2 flex gap-2">
            <div @click="reject(reportData.id,'reject')" style="background-color: #e5737333" class="flex items-center gap-2 p-3 rounded-full">
                <red-trash width="22" height="22" />
            </div>
            <div @click="approve(reportData.id, 'approve')" class="flex items-center gap-2 bg-[#7FB77E33] p-3 rounded-full">
